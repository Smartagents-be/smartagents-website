...
// .wrangler/tmp/pages-Oiz0BS/functionsWorker-0.5546473950058357.mjs
...
async function generateToken2(secret) {
  const encoder = new TextEncoder();
  const key = await crypto.subtle.importKey(
    "raw",
    encoder.encode(secret),
...
async function onRequest2(context) {
  ...
  if (token && env.EXPORT_SESSION_SECRET) {
    const expected = await generateToken2(env.EXPORT_SESSION_SECRET);
    if (timingSafeEqual2(token, expected)) {
...
// .wrangler/tmp/bundle-0CpuxK/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-0CpuxK/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.5546473950058357.js.map
