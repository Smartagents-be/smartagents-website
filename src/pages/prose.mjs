...
//
// Two page families run long enough to need their own heading, quote and list
// levels — the insights and the privacy notice — so they get a small authoring
// vocabulary rather than hand-written markup, and it sits here because it
// belongs to neither. `p`, `h2` and `quote` are tagged template literals, which
// is what lets a Dutch or French sentence carry its apostrophes without a
// backslash; `**bold**` and `[label](href)` are the only inline marks, and a
// href of the form `insight:<key>` is resolved by the caller.
//
// Every element carries an id (element-ids §1). A paragraph has no content key
// of its own, so the block index is the key. Reordering a body renumbers it,
// which is the one place this repo accepts that: prose is read in order and
// nothing outside the article points into it.
 ...
 *
 * Interpolations are interleaved rather than dropped — `strings.join('')`
 * ignores the values entirely, which is silent, and silent is the wrong failure
 * for a tag whose job is to carry a sentence. The privacy notice interpolates a
 * VAT number and an e-mail address and lost both without a murmur.
 */
...
// use characters escapeHtml leaves alone, and everything it captures is already
// escaped by the time it is re-inserted.
const INLINE = /\[([^\]]+)\]\(([^)]+)\)|\*\*([^*]+)\*\*/g;
      ...
      switch (block.type) {
        case 'h2':
          /* `tabindex="-1"` so a link into the body moves focus and not only the
             ...
             to a fragment's target; Safari does not, so a keyboard user who
             follows the clause index and presses Tab resumes at the index. */
