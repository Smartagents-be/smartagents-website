...
(function () {
  ...
  const you      = document.getElementById('off-you');
  const youWait  = document.getElementById('off-you-wait');
  const youDone  = document.getElementById('off-you-done');
  ...
  function reset() {
    [a1, summary, a2, you].concat(srcs).concat(fields).forEach((el) => el && el.classList.remove('is-in'));
    srcs.forEach((s) => s.classList.remove('loading', 'done'));
    badge.classList.remove('is-in');
    youWait.classList.remove('is-in');
    youDone.classList.remove('is-in');
  ...
  async function loop() {
    ...
    while (gen === myGen) {
      ...
      await wait(1190); if (gen !== myGen) return;
      you.classList.add('is-in');                        // ③ Jij waits
      youWait.classList.add('is-in');
      await wait(1330); if (gen !== myGen) return;
