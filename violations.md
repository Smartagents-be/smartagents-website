...
asserts `accessibility: 1` and `cumulative-layout-shift: 0`. **Both are met
now.** CLS was re-measured over the Layout Instability API after the #6 fix and
is exactly 0 on `/nl/`, `/nl/training/`, `/nl/ai-staffing/`, `/nl/team/`,
`/nl/inzichten/` and an article, with FCP and LCP unchanged (~50ms against a
local server, three runs each). Lighthouse SEO 100 only means the basics are
...
Tick a box when the item ships.

**Second pass, 2026-09-03.** Every open item was worked through. What is left is
three boxes, and none of them is code:

- **#2 `[~]`** — the endpoint is correct and fails loudly; the binding is still
  missing. `N8N_WEBHOOK_URL` is in no commit on any branch — it exists only in
  the Cloudflare Pages dashboard, which Cloudflare ignores once a project has a
  `wrangler.toml`. **This needs the URL pasted into a `[vars]` block in
  `wrangler.toml`, and nobody but the client has it.** Until then every
  submission answers 502, which is the right failure and not a working form.
- **#25 `[~]`** — every course now carries a facts strip: who it is for, the
  format, the group size, what to bring, the price. All five are read off
  something the site already said. **Duration and open dates are business facts
  nothing in the repo knows and they are deliberately absent rather than
  invented.** Give us a duration per course and a date list and they go in.
- **#26 `[~]`** — the FAQ half is built (six questions on the homepage, open, in
  three languages, and the site's `FAQPage` structured data). **The proof half
  is not, and cannot be: a case study, a testimonial, a client logo and a
  measurable result are records, and writing them without the client's material
  and permission would be fabricating them.** Send one real engagement and one
  approved quote and both go in the week they arrive.

Two items were checked against the build and closed without a change:

- **#27 `[-]`** — naming Anthropic's playbook is deliberate; confirmed
  2026-09-03. The line stays.
- **#35 `[-]`** — **not reproduced.** Measured and photographed at 360, 390 and
  620px on `/nl/` and `/nl/ai-staffing/`: `#heroSwoop` is not "a shallow sliver
  across the top" but a purpose-drawn shape running the whole right flank, from
  the top edge past the last line of copy, carrying its own node field. It takes
  about a third of the hero's width at its widest. The one true half of the item
  is that the phone has one shape where the desk has two, which is a decision
  the CSS states and defends ("the counter-lobe has nothing left to answer once
  the petal has stopped being a flank"); adding a second dark mass behind a
  phone's body copy would cost more than it buys. Screenshots on request.

Everything else is `[x]`, and the ones worth reading about:

- **#6** — CLS is **0**, measured over the Layout Instability API on `/nl/`,
  `/nl/training/`, `/nl/ai-staffing/`, `/nl/team/`, `/nl/inzichten/` and an
  article, with FCP/LCP unchanged at ~50ms locally. The fix is in `src/motion.js`
  and it is not the one suggested here. Reserving the box in CSS cannot work:
  the silhouette is a `clip-path` in `objectBoundingBox` units, so a box grown
  before the outline is remapped renders the shape stretched. Loading motion.js
  before the paint does work but cost 100ms of FCP, because sampling five
  outlines with `getPointAtLength` is 111ms on a cold engine. So the setup is
  split: `collectMagnets()` grows the box and writes the resting silhouette by
  affinely remapping the *authored* Bézier (exact, ten segments, no sampling),
  and `arm()` does the sampling on the first idle callback or the first pointer
  move. Nothing needs the dense outline until a cursor arrives.
- **#7 / #8 / #9** — `src/layouts/schema.mjs` is new: `Organization` and
  `WebSite` on every page with stable `@id`s, plus `Service` ×4, `BlogPosting`
  ×4, `Person` ×2, `Blog`, `FAQPage` and `BreadcrumbList` where they belong. A
  page contributes its own nodes by exporting `schema({ t, lang, url })`. The
  share card and the square `Organization.logo` are generated from the site's own
  tokens by `scripts/make-social-images.mjs` (headless Chrome, committed output,
  never run in CI); an article overrides the card with its own thumbnail and sets
  `og:type=article` with the published date the body only had as a `<time>`.
- **#10** — `/nl/inzichten/`, `/en/insights/`, `/fr/analyses/` exist, and the
  homepage section and the index print the same rows from the same function, so
  they cannot drift. The rail's "Alle artikelen →" and the nav sheet's
  "Inzichten" both point at it now.
- **#20** — finished, and verified rather than asserted: a DOM walk over every
  public page reports **0** elements inside `<body>` without an id, on all nine
  templates. That includes the logo mark's own parts, the chevron, the burger
  bars, the language chips' two spans and all 32 isometric plane cells.
- **#23** — `robots.txt` names fifteen AI crawlers explicitly and `/llms.txt` is
  generated from the same page modules and string files the site is.
- **#26 (the half that shipped) and #14** — every hero has a lede now, and the
  homepage's wordmark is a `<p>` beside the heading rather than the first line
  inside it. Nothing moved on screen.
- **#33** — verified in a browser, not just written: empty submit names all three
  fields at once, a malformed address gets the e-mail message, and correcting a
  field clears its message on `input`.

The original first-pass notes follow.
...
Nothing here is blocked on code any more. What is left is three things only the
client can supply, in the order they are worth having:

1. **The n8n webhook URL** (#2). Paste it into a `[vars]` block in
   `wrangler.toml` and the contact form works. Until then it answers 502, and
   the site should not be pointed at traffic.
2. **Proof** (#26). One real engagement written as situation → what we did →
   measurable result, and two quotes with permission to print them and a name.
   The FAQ half is live; this is the half that changes whether the site
   converts.
3. **The two training facts** (#25). A duration per course and a list of open
   dates. The strip that would carry them is already on the page.
