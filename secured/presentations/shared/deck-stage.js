...
(() => {
  ...
  const stylesheet = `
    ...
    .overlay[data-presenting] {
      opacity: 0 !important;
      pointer-events: none !important;
      transform: translate(-50%, 6px) scale(0.92) !important;
  ...
  class DeckStage extends HTMLElement {
    ...
    _enableRail() {
      ...
      // Per-viewer preference — restored alongside rail width. Default hidden;
    ...
    _render() {
      ...
      overlay.innerHTML = `
        ...
        <button class="btn rail-toggle" type="button" aria-label="Toggle thumbnail rail" title="Toggle rail">
          <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="2" y="2" width="4" height="12" rx="1"/><path d="M9 5h4M9 8h4M9 11h4"/></svg>
        </button>
      `;
    ...
    _onFullscreenChange() {
      if (document.fullscreenElement && this._railVisible) {
        this._railVisible = false;
    ...
    _onMessage(e) {
      ...
      if (d && typeof d.__omelette_presenting === 'boolean') {
        ...
        if (this._overlay) {
          if (this._presenting) {
            this._overlay.removeAttribute('data-visible');
            this._overlay.setAttribute('data-presenting', '');
            if (this._hideTimer) clearTimeout(this._hideTimer);
          } else {
            this._overlay.removeAttribute('data-presenting');
          }
        }
        this._syncRailHidden();
        this._closeMenu();
